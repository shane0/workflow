...
# Set up the logger
from plugins.mods.log_tools import setup_logger, log_dir
# logger = setup_logger('cli_logger', logging.DEBUG)
logfilename = 'cli_logs.csv'
log_file = os.path.join(log_dir, logfilename)
logger = setup_logger(log_file,logging.DEBUG)
...
def before_invoke(self, ctx):
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2:
            logger.info(f"cli.py {sys.argv[1]} {sys.argv[1]}")
        logger.info(f"Loading command: {sys.argv[1]}")  # Log the command being loaded
